<template>
  <div
    class="pl-5 group relative grid pb-1 transition-all sm:grid-cols-8 sm:gap-8 md:gap-4 lg:hover:!opacity-100 lg:group-hover/list:opacity-50"
  >
    <div
    ...
    <header
      ...
      :aria-label="`${periods[0]} to ${periods[1]}`"
    >
      {{ periods[0] }} — {{ periods[1] }}
    </header>
        ...
        <div>
          ...
          >
            ...
            <span class="font-medium"
              >{{ position }} ·<!-- -->
              <span class="inline-block pl-1 font-medium">
                {{ company }}
                <ArrowUpRightIcon />
              </span>
            </span>
